...
import { getAllUsersLocked } from "@/stores/actions/userActions";
import { Link } from "react-router-dom";

const LockedUser = () => {
  const dispatch = useDispatch();
  const { getUsersLock, loading } = useSelector((state: any) => state.users);
  ...
  console.log(getUsersLock);

  const users = getUsersLock.data || [];
  const pagination = getUsersLock?.pagination;
  ...
  const handleUnlockUser = async () => {
    if (!selectedUser) return;
    try {
      await authService.restoreUser(selectedUser.id);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-semibold">Danh sách người dùng bị khóa</h1>
